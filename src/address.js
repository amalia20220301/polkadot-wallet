...
export const getKeyPair = (word, coinType)=>{
    const path = `m/44'/${coinType}'/0'/0'/1'`
    const seed = bip39.mnemonicToSeedSync(word)
    const { key } = derivePath(path, seed.toString('hex'));
    const pubKey = getPublicKey(key, false);
    return {
        publicKey: pubKey.toString('hex'),
        secretKey: `${key.toString('hex')}${pubKey.toString('hex')}`,
    }
}
...
// getSnapAddress(process.env.kidney, 1)
